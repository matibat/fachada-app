/**
 * BDD: pages content collection Zod schema
 *
 * Tests the pagesSchema in isolation (no Astro runtime needed).
 * Uses schema.safeParse() so the test runner never hits astro:content.
 */
import { describe, it, expect } from "vitest";
import { pagesSchema } from "../src/content/pages.schema";

// ─── Fixtures ────────────────────────────────────────────────────────────────

const minimalValid = {
  title: "Getting Started",
  description: "How to use fachada.",
...
describe("pages collection schema", () => {
  ...
  describe("Behavior: accepts entry with all optional fields populated", () => {
    it("Given a page object with every field set, When parsed, Then safeParse returns success and data matches input", () => {
      ...
      expect(result.data.title).toBe("Full Page");
      expect(result.data.apps).toBe("*");
      ...
      expect(result.data.keywords).toEqual(["astro", "fachada"]);
      expect(result.data.downloadFilename).toBe("guide.pdf");
    });
  });

  // ─── Behavior: optional fields default to undefined when absent ────────────
